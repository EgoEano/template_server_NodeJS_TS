...
const allowedUserAgentName: string = process.env.USER_AGENT_NAME || '';
const allowedUserAgentVersion: string = process.env.USER_AGENT_VERSION || '1.0.0';
const allowedUserAgent: string = `X-${allowedUserAgentName}-Client/${allowedUserAgentVersion}`;
...
export function userAgentWhiteList(app: Express): void {
  app.use((req: Request, res: Response, next: NextFunction): void => {
    const ua: string = (req.headers['user-agent'] || '').toLowerCase().trim();
    if (
        !ua || ua.length === 0 || ua !== allowedUserAgent
    ) {
        Logger.warn({
            message: `Blocked User-Agent. ip: ${req.ip}, Agent: ${ua}`,
